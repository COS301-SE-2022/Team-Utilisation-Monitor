...
export class AuthenticationService {
  ...
  */
  ...
  login(username:string,password:string):Observable<any>
  {
    ...
    const obj= this.client.post<any>('http://ec2-52-90-69-184.compute-1.amazonaws.com:6565/graphql',JSON.stringify({ query: query }), options);
  ...
  */
  ...
  registerUser(name:string,surname:string,username:string,password:string):Observable<any>
  {
    ...
    const object=this.client.post<any>('http://ec2-52-90-69-184.compute-1.amazonaws.com:6565/graphql',JSON.stringify({ query: Query }), options)
    return object;
  }
